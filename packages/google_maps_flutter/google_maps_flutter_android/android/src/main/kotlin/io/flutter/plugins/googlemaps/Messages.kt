    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: MapsApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel =
            BasicMessageChannel<Any?>(
                binaryMessenger,
                "dev.flutter.pigeon.google_maps_flutter_android.MapsApi.getClusters$separatedMessageChannelSuffix",
                codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
...
class MapsCallbackApi(
  ...
  fun onClusterManagersUpdated(
      clusterManagerIdArg: String,
      clustersArg: List<PlatformCluster>,
      callback: (Result<Unit>) -> Unit
  ) {
    val separatedMessageChannelSuffix =
        if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName =
        "dev.flutter.pigeon.google_maps_flutter_android.MapsCallbackApi.onClusterManagersUpdated$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(clusterManagerIdArg, clustersArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlutterError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
