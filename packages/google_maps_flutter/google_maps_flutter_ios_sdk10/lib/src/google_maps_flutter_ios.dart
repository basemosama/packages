...
class GoogleMapsFlutterIOS extends GoogleMapsFlutterPlatform {
  ...
  Future<List<Cluster>> getClusters({
    ...
    final List<PlatformCluster> clusters = await _hostApi(
      mapId,
    ).getClusters(clusterManagerId.value);
    return clusters.map((PlatformCluster cluster) => clusterFromPlatformCluster(cluster)).toList();
  }
...
class HostMapMessageHandler implements MapsCallbackApi {
  ...
  @override
  void onClusterManagersUpdated(String clusterManagerId, List<PlatformCluster> clusters) {
    streamController.add(
      ClusterManagerUpdateEvent(
        mapId,
        clusters
            .map(
              (PlatformCluster cluster) => GoogleMapsFlutterIOS.clusterFromPlatformCluster(cluster),
            )
            .toList(),
