  ...
  /// Returns the clusters for the given cluster manager.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FGMGetGoogleMapsFlutterPigeonMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(clustersWithIdentifier:error:)], @"FGMMapsApi api (%@) doesn't respond to @selector(clustersWithIdentifier:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
...
- (void)didUpdateClusterManagersWithIdentifier:(NSString *)arg_clusterManagerId clusters:(NSArray<FGMPlatformCluster *> *)arg_clusters completion:(void (^)(FlutterError *_Nullable))completion {
  NSString *channelName = [NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.MapsCallbackApi.onClusterManagersUpdated", _messageChannelSuffix];
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:channelName
      binaryMessenger:self.binaryMessenger
      codec:FGMGetGoogleMapsFlutterPigeonMessagesCodec()];
  [channel sendMessage:@[arg_clusterManagerId ?: [NSNull null], arg_clusters ?: [NSNull null]] reply:^(NSArray<id> *reply) {
    if (reply != nil) {
      if (reply.count > 1) {
        completion([FlutterError errorWithCode:reply[0] message:reply[1] details:reply[2]]);
      } else {
